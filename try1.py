...
class MLP(nn.Module):
    def __init__(self, args):
        ...
        self.lstm = nn.LSTM(input_size=self.d_input, hidden_size=self.d_input//2, num_layers=1, batch_first=True)
    ...
    def forward(self, x, val=False):
        ...
        h1 = torch.zeros(1, batch_size, self.d_input//2).to(x.device)
        c1 = torch.zeros(1, batch_size, self.d_input//2).to(x.device)
