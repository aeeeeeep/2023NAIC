...
class MLP(nn.Module):
    def __init__(self, args):
        ...
        # self.lstm2 = nn.LSTM(input_size=self.d_input//16, hidden_size=self.d_input//32, num_layers=1, batch_first=True)
        ...
        self.classifier1 = torch.nn.Sequential(
                nn.Linear(self.d_input//16, self.d_input//32, bias=False),
                nn.BatchNorm1d(5),
                nn.ReLU(),
                nn.Dropout(p=0.1),
                nn.Linear(self.d_input//32, self.d_input//64, bias=False)
            )
    ...
    def forward(self, x, val=False):
        ...
        # h2 = torch.zeros(1, batch_size, self.d_input//32).to(x.device)
        # c2 = torch.zeros(1, batch_size, self.d_input//32).to(x.device)
